...
public class TestInfoPotentialPathManipulation extends AbstractAppParamPlugin {
	...
	// private static Pattern patternFilePath1 =
	// Pattern.compile("\\A((/)[\\w\\.\\-]+)+(/)?\\z", PATTERN_PARAM);
	// private static Pattern patternFilePath2 =
	// Pattern.compile("\\A[\\w\\.\\-]+((/)[\\w\\.\\-]+)+\\z", PATTERN_PARAM);

	private static Pattern[] patternFilePath = {
			Pattern.compile("\\A((/)[\\w\\.\\-~]+)+(/)?\\z", PATTERN_PARAM),
			Pattern.compile("\\A[\\w\\.\\-~]+((/)[\\w\\.\\-~]+)+\\z",
					PATTERN_PARAM),
			Pattern.compile("\\A((\\\\)[\\w\\.\\-~]+)+(\\\\)?\\z",
					PATTERN_PARAM),
			Pattern.compile("\\A[\\w\\.\\-]+((\\\\)[\\w\\.\\-~]+)+\\z",
					PATTERN_PARAM)
	...
	public void scan(HttpMessage msg, String param, String value) {
		...
		for (int i = 0; i < patternFilePath.length; i++) {
			...
			if (matcher.find()) {
				...
				bingo(Alert.RISK_INFO, Alert.SUSPICIOUS, "",
						(query == null || query.length() == 0) ? "nil" : query,
						matchedFilePath, msg);
				return;
			}
		}
		...
		// if (matcher.find()) {
		// matchedFilePath = matcher.group(0);
		// bingo(Alert.RISK_INFO, Alert.SUSPICIOUS, "", (query == null ||
		// query.length() == 0)? "nil" : query, matchedFilePath , msg);
		//            
		// }
