...
public class TableAlert extends AbstractTable {
	...
	public Vector<Integer> getAlertListByScan(int scanId) throws SQLException {
		PreparedStatement psReadScan = getConnection().prepareStatement(
				"SELECT ALERTID FROM ALERT WHERE " + SCANID + " = ?");
	...
	public Vector<Integer> getAlertListBySession(long sessionId) throws SQLException {
		...
		PreparedStatement psReadSession = getConnection()
				.prepareStatement(
						"SELECT ALERTID FROM ALERT INNER JOIN SCAN ON ALERT.SCANID = SCAN.SCANID WHERE SESSIONID = ?");
