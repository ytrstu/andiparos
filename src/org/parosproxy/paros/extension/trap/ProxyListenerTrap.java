...
public class ProxyListenerTrap implements ProxyListener {
	...
	public void onHttpRequestSend(HttpMessage msg) {
		...
		if (!getTrapPanel().getChkTrapRequest().isSelected() && !getTrapPanel().getChkTrapRequestOnce().isSelected()) {
			return;
		}
		...
		getTrapPanel().getChkTrapRequestOnce().setSelected(false);
	}
		...
		try {
			EventQueue.invokeAndWait(new Runnable() {
				public void run() {
				    ...
				    if (getTrapPanel().flagPackage()) {
						msg.setFlag(true);
						getTrapPanel().setFlagPackage(false);
					}
