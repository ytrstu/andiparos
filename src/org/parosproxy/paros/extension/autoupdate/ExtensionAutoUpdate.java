...
public class ExtensionAutoUpdate extends ExtensionAdaptor {
	...
	private Pattern patternNewestVersionWindows = Pattern.compile(
			"paros-(\\d+)\\.(\\d+)\\.(\\d+)-win\\.dat", Pattern.MULTILINE);
	private Pattern patternNewestVersionLinux = Pattern.compile(
			"paros-(\\d+)\\.(\\d+)\\.(\\d+)-unix\\.zip", Pattern.MULTILINE);
									...
									EventQueue.invokeLater(new Runnable() {
										public void run() {
											...
											if (newestVersionName == null) {
												...
												int result = getView()
														.showConfirmDialog(
																"There is a newer version of Paros: "
																		+ newestVersionName
																				.replaceAll(
																						"\\.dat",
																						"")
												...
												if (result == JOptionPane.OK_OPTION) {
													waitDialog = getView()
															.getWaitMessageDialog(
																	"Download in progress...");
													Thread t = new Thread(
															new Runnable() {
																public void run() {
																	ExtensionAutoUpdate.this
																			.download(false);
																}
															});
													t.start();
	...
	public void hook(ExtensionHook extensionHook) {
		...
		if (getView() != null) {
			extensionHook.getHookMenu().addToolsMenuItem(
					getMenuItemCheckUpdate());
		}
	}
	...
	public void download(final boolean silent) {
		...
		try {
			...
			msg
					.setRequestBody(getBody(
							mirrorList[getRandom(mirrorList.length)],
							newestVersionName));
				...
				EventQueue.invokeAndWait(new Runnable() {
					public void run() {
						...
						if (!silent) {
							...
							if (Constant.isLinux()) {
								s = s
										+ "  Note: Use startserver.sh to run Paros.";
							}
				...
				EventQueue.invokeAndWait(new Runnable() {
					public void run() {
						...
						if (!silent) {
							...
							getView()
									.showWarningDialog(
											"Error encountered.  Please download new updates manually");
						}
					}
				});
			} catch (Exception e1) {
	...
	private String getNewestVersionName() {
		...
		try {
			...
			if (matcher.find()) {
				...
				long version = 10000000 * ver_major + 10000 * ver_minor
						+ ver_release;
	...
	private HttpSender getHttpSender() {
		if (httpSender == null) {
			httpSender = new HttpSender(getModel().getOptionsParam()
