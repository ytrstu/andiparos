...
public class HttpInputStream extends BufferedInputStream {
	...
	/**
	 * Check if the current StringBuffer trailing characters is an HTTP header
	 * end (empty CRLF).
	 * 
	 * @param sb
	...
	/**
	 * Read Http body from input stream as a string basing on the content length
	 * on the method.
	 * 
	 * @param method
	 * @return Http body
	 ...
	 * 
	 * @param contentLength
	 *            Content length read to be read. -1 = unlimited until
	 *            connection close.
	 * @param readBodyLength
	 *            Body length read so far
	 * @param data
	 *            Buffer storing the read bytes.
	 * @return Numfer of bytes read in buffer
	 * @throws IOException
	 */
	private int readBody(int contentLength, int readBodyLength, byte[] buffer)
			throws IOException {
