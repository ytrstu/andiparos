...
public class Spider {
	...
	void checkIfAllThreadCompleted() {
		for (int i = 0; i < spiderThread.length; i++) {
			if (!spiderThread[i].isCompleted()) {
				// Cannot check thread alive here because child issued this
				// check. They will not stop yet.
				return;
			}
		}
	...
	synchronized void SpiderProgress(QueueItem item) {
		...
		percentage += scale * (depthQueueCount[item.getDepth()] - outstanding)
						/ depthQueueCount[item.getDepth()];

		try {
			notifyListenerSpiderProgress(item.getMessage().getRequestHeader()
					.getURI(), percentage);
			Thread.sleep(100);
		} catch (Exception e) {
			// ZAP: Log exceptions
	...
	private void notifyListenerSpiderProgress(URI uri, int percentageComplete) {
		...
		for (int i = 0; i < listenerList.size(); i++) {
			...
			listener.spiderProgress(uri, percentageComplete,
				visitedPostMethod.size() + visitedGetMethod.size(),
				queue.size());
		}
	}
	...
	boolean isInVisitedLink(HttpMessage msg) {
		...
		try {
			if (model.getDb().getTableHistory().containsURI(
	...
	boolean isQueued(HttpMessage msg) {
		...
		try {
			return model.getDb().getTableHistory().containsURI(
					session.getSessionId(), HistoryReference.TYPE_SPIDER_SEED,
					msg.getRequestHeader().getMethod(),
					msg.getRequestHeader().getURI().toString(),
					msg.getRequestBody().toString());
	...
	/**
	 * Check if URL is to be neglected if: - not HTTP protocol - outside host
	 * domain - irrelevant file suffix (eg gif, jpg) - visited before URL
	 * queried by this method will be marked visited.
	 * 
	 * @throws URIException
	 * 
	 */
