...
public class TestInjectionSQLFingerprint extends AbstractAppParamPlugin {
	...
	private static final String MSSQL_DELAY_1 = "';waitfor delay '0:0:05';--";
	private static final String MSSQL_DELAY_2 = ";waitfor delay '0:0:05';--";
	...
	/*
	 * invalid statement to trigger SQL exception. Make sure the pattern below
	 * does not appear here
	 */

	private static final Pattern patternErrorODBC1 = Pattern.compile(
			"Microsoft OLE DB Provider for ODBC Drivers.*error", PATTERN_PARAM);
	private static final Pattern patternErrorODBC2 = Pattern.compile(
			"ODBC.*Drivers.*error", PATTERN_PARAM);
	private static final Pattern patternErrorGeneric = Pattern.compile(
			"JDBC|ODBC|not a valid MySQL|SQL", PATTERN_PARAM);
	...
	private void testMSBlindINSERT(HttpMessage msg, String param, String value)
			throws HttpException, IOException {
		...
		for (int i = 0; i < TRY_COUNT; i++) {
			...
			// try INSERT
			newQuery = setParameter(msg, param, value + "'"
					+ sbInsertValue.toString() + SQL_BLIND_MS_INSERT);

			sendAndReceive(msg);

			if (checkMSTimeResult(msg, newQuery, defaultTimeUsed, msg
	...
	private boolean checkResult(HttpMessage msg, String query) {
		...
		if (matchBodyPattern(msg, patternErrorODBC1, sb)
			...
			bingo(Alert.RISK_HIGH, Alert.SUSPICIOUS, null, query,
					sb.toString(), msg);
			isSqlError = true;
		...
		if (matchBodyPattern(msg, patternErrorODBCMSSQL, sb)) {
			getKb().add(msg.getRequestHeader().getURI(), "sql/mssql",
					new Boolean(true));
			return true;
		}
	...
	private boolean checkMSTimeResult(HttpMessage msg, String query,
			long defaultTimeUsed, long timeUsed) {
