 ...
 * (header and body) pair.
 * 
 */
public class HttpMessage {

	private static Pattern staticPatternParam = Pattern.compile("&(amp;)?", Pattern.CASE_INSENSITIVE);
	 ...
	 * @param resBody
	 */
	public HttpMessage(HttpRequestHeader reqHeader, HttpBody reqBody, HttpResponseHeader resHeader, HttpBody resBody) {
		setRequestHeader(reqHeader);
		setRequestBody(reqBody);
	...
	public HttpMessage(String reqHeader, String reqBody, String resHeader, String resBody)
	 ...
	 * 
	 * @return response. if getResponseHeader().toString() is "" then it has not
	 *         been read yet.
	 */
	...
	public void setRequestHeader(String reqHeader)
	throws HttpMalformedHeaderException {
		HttpRequestHeader newHeader = new HttpRequestHeader(reqHeader);
		setRequestHeader(newHeader);
	}

	public void setResponseHeader(String resHeader)
	throws HttpMalformedHeaderException {
	...
	/**
	 * Compare if 2 message is the same. 2 messages are the same if: Host, port,
	 * path and query param and VALUEs are the same. For POST request, the body
	 * must be the same.
	 * 
	 * @param msg
	 * @return
	...
	public boolean equals(Object object) {
		...
		// compare method
		if (!this.getRequestHeader().getMethod().equalsIgnoreCase(
		...
		try {
			...
			if (this.getRequestHeader().getMethod().equalsIgnoreCase(HttpRequestHeader.POST)) {
				return this.getRequestBody().toString(HttpBody.STORAGE_CHARSET)
					.equalsIgnoreCase(msg.getRequestBody().toString(HttpBody.STORAGE_CHARSET));
			}
	...
	/**
	 * 2 messages are equal type if the host, port, path and query names are
	 * equal. Even though the query value may differ.
	 * 
	 * @param msg
	 * @return
	 */
	...
	private boolean queryEquals(HttpMessage msg) throws URIException {
		...
		if (uri1.getQuery() != null)
			query1 = uri1.getQuery();
		if (uri2.getQuery() != null)
			query2 = uri2.getQuery();
	...
	public TreeSet<String> getParamNameSet(String params) {
		...
		for (int i = 0; i < keyValue.length; i++) {
			...
			try {
				// Also handle Query String Parameters without a value
				if (!keyValue[i].isEmpty() && keyValue[i].indexOf('=') == -1) {
					key = keyValue[i];
				} else if (pos > 0) {
					// param found
	...
	/**
	 * @return Get the elapsed time (time difference) between the request is
	 *         sent and all response is received. In millis. The value is zero
	...
	/**
	 * Set the elapsed time (time difference) between the request is sent and
	 * all response is received. In millis.
	 * 
	 * @param timeElapsed
	 */
	 ...
	 * System.currentTimeMillis before the message is sent. The value is zero if
