...
public class FilterDetectMalciousContent extends FilterAdaptor {
	 ...
	 * (non-Javadoc)
	 * 
	 * @seecom.proofsecure.paros.extension.filter.Filter#onHttpRequestSend(com.
	 * proofsecure.paros.network.HttpMessage)
	 */
	 ...
	 * 
	 * @see
	 * com.proofsecure.paros.extension.filter.Filter#onHttpResponseReceive(com
	 * .proofsecure.paros.network.HttpMessage)
	 */
		...
		while (matcherForm.find()) {
			...
			if (txtForm != null && txtInput != null) {
				...
				writeOutput(
						...
						"Password field without setting autocomplete to off found.",
						txtInput);
	...
	private static final int DEFAULT = Pattern.MULTILINE
	...
	private final static Pattern[] patternBadBodyList = {
			...
			Pattern.compile(
					"<[^>]*CLSID:11111111-1111-1111-1111-11111111111[^>]*>",
					DEFAULT),
			...
			Pattern.compile(
					"^.*?CreateObject\\(\\s*[\"']+Adodb.Stream[\"']\\s*\\)$",
					DEFAULT),
			Pattern.compile("^.*?execcommand.*?$", DEFAULT),
			Pattern.compile("(ms-its|ms-itss|mk:@MSITStore):mhtml:file://",
					DEFAULT),
			Pattern.compile("ms-its|ms-itss", DEFAULT), // simplified one of the
														// above
	...
	private void checkMaliciousCode(HttpMessage msg) {
		...
		for (int i = 0; i < patternBadHeaderList.length; i++) {
			...
			while (matcher.find()) {
				writeOutput(msg.getRequestHeader().getURI().toString(),
						patternBadHeaderDesc[i], matcher.group(0));
			}
		}
		...
		for (int i = 0; i < patternBadBodyList.length; i++) {
			...
			while (matcher.find()) {
				writeOutput(msg.getRequestHeader().getURI().toString(),
						patternBadBodyDesc[i], matcher.group(0));
			}
		}
	}
