...
public class PopupMenuExportMessage extends ExtensionPopupMenu {
			...
			public void actionPerformed(java.awt.event.ActionEvent e) {
				...
				if (obj.length == 0) {
					extension
							.getView()
							.showWarningDialog(
									"Select HTTP messages in History panel before export to file.");
					return;
				}
				...
				if (file.exists()) {
					int rc = extension.getView().showYesNoCancelDialog(
							"File exists.  Yes = overwrite, No = append?");
	...
	private File getOutputFile() {
		...
		JFileChooser chooser = new JFileChooser(extension.getModel()
				.getOptionsParam().getUserDirectory());
		...
		if (rc == JFileChooser.APPROVE_OPTION) {
			...
			extension.getModel().getOptionsParam().setUserDirectory(
